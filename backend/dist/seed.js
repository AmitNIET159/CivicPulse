...
async function seed() {
    try {
        ...
        await Promise.all([
            ...
            Issue_1.default.deleteMany({}),
            Vote_1.default.deleteMany({}),
        ]);
        console.log('🗑️  Cleared existing data');
        // Create admins
        const admins = await User_1.default.create([
        ...
        // Create officials
        const officials = await User_1.default.create([
        ...
        const citizens = await User_1.default.create(citizenNames.map((name, i) => ({
            name,
        ...
        await Issue_1.default.insertMany(issues);
        console.log('📋 Created 50 sample issues across Delhi');
        // Add some votes
        const createdIssues = await Issue_1.default.find().lean();
        const voteDocs = [];
        for (const issue of createdIssues) {
            ...
            await Issue_1.default.findByIdAndUpdate(issue._id, { voters: voterIds });
            for (const voterId of voterIds) {
                voteDocs.push({ issue: issue._id, user: voterId });
            }
        }
        if (voteDocs.length > 0) {
            await Vote_1.default.insertMany(voteDocs, { ordered: false }).catch(() => { });
        }
